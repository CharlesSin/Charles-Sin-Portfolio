      ...
      <div className={cx("col-span-12 sm:col-span-5 text-right sm:absolute sm:left-[50%]", secondary && "!relative z-[100] !left-0 !text-start sm:w-[120%]")}>
        <div>
          ...
          <div className={cx("flex justify-end mt-5 gap-3 text-slate-300", secondary && "!justify-start")}>
            {live && (
              <Link href={`${live}`} target="_blank" className="hover:text-sky-400 transition-all duration-300 hover:scale-110 text-2xl">
                <HiOutlineExternalLink />
              </Link>
            )}

            {code && (
              <Link href={`${code}`} target="_blank" className="hover:text-sky-400 transition-all duration-300 hover:scale-90 text-[1.2rem]">
                <FiGithub />
              </Link>
            )}
          </div>
